...
subcategoriaModel.getAllSubcategoria = function( callback ){
	
	if (connection){
		connection.query(' SELECT * FROM subcategoria WHERE id_subcategoria != 0 ORDER BY id_subcategoria ASC  ', function(error, rows){
			if(error){
				throw error;
			}else{
				callback(null, rows);
